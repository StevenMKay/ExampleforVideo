...
class CallCenterApp {
    ...
    updateChatHeader() {
        ...
        if (this.isVerified) {
            ...
            // Refresh profile to show account balances
            this.loadCustomerProfile();
        } else {
    ...
    loadCustomerProfile() {
        ...
        if (customer.dob) {
            ...
            // Update account balances with verification check
            if (this.isVerified) {
            ...
            // Set default balances with verification check
            if (this.isVerified) {
    ...
    sendMessage() {
        ...
        // Check if agent is trying to access sensitive info without verification
    ...
    addMessage(content, sender) {
        ...
        // Auto-verify if customer provides correct verification info in chat
    ...
    checkForAutoVerification(customerMessage) {
        if (!this.currentCustomer || this.isVerified) return;
        ...
        const dobCheck = message.includes(this.currentCustomer.dob) || 
                        ...
                        message.includes(new Date(this.currentCustomer.dob).toLocaleDateString('en-US', {year: 'numeric', month: 'long', day: 'numeric'}));
        if (dobCheck) {
            verificationCount++;
        }
        ...
        // If customer provided sufficient verification info
    ...
    generateCustomerResponse(agentMessage) {
        ...
        // Find appropriate response based on agent message and current state
        let response = '';
        ...
        if (lowerMessage.includes('mother') && lowerMessage.includes('maiden')) {
            ...
            // Fix timezone issue by adding time component and using proper date formatting
            const dobDate = new Date(this.currentCustomer.dob + 'T00:00:00');
            ...
            if (!this.isVerified) {
                ...
                if (scenario.responses && scenario.responses.amount_check) {
                    response = "Yes, that sounds right. " + (scenario.responses.amount_check || "Can we proceed with the transaction?");
                } else {
                    response = "That sounds correct. What do we need to do next?";
                }
            }
        } else if (lowerMessage.includes('thank')) {
            response = "Thank you so much for your help! This really means a lot to me.";
        } else if (this.isVerified) {
            // Use scenario-specific responses only after verification
    ...
    verifyCustomer() {
        ...
        // Improved date comparison - handle different date formats
        if (customer.dob && dobInput) {
            // Parse both dates carefully to avoid timezone issues
            const inputDate = dobInput; // This is already in YYYY-MM-DD format from HTML date input
            const customerDob = customer.dob; // This is stored as YYYY-MM-DD
            
            if (inputDate !== customerDob) {
                // Convert to user-friendly format for error message
                const customerDate = new Date(customerDob + 'T00:00:00'); // Add time to avoid timezone issues
                const expectedDateStr = customerDate.toLocaleDateString('en-US');
                alert(`Date of birth does not match our records. Expected: ${expectedDateStr} (Enter as: ${customerDob})`);
                return;
            }
        }
